...
async function TaskList ({appType, appAreas, packageType, state = 0, page = 1, row = 12, groupId}) {
    const res = await axios.post(`/task/list/${page}/${row}`, JSON.stringify({
        appType,
        appAreas,
        state,
        packageType,
...
async function TaskAdd (state, {projectName, packageType, totalType, total, appAreas, vevironments, appType, groupId, taskEndTime, logo, referImg, remarks}) {
    ...
    const res = await axios.post(`/task/save/${state}`, {
        projectName,
        packageType,
        totalType,
        total,
...
async function TaskSave (state, {id, projectName, packageType, totalType, total, appAreas, vevironments, appType, groupId, taskEndTime, logo, referImg, remarks}) {
    taskEndTime = moment(taskEndTime).format("YYYY-MM-DD HH:mm:ss");
    const res = await axios.post(`/task/update/${state}`, {
        id,
        projectName,
        packageType,
        totalType,
...
async function taskWall ({appType, appAreas, packageType, state, page, row}) {
    const res = await axios.post(`/taskWall/list/${page}/${row}`, JSON.stringify({
        appType,
        appAreas,
        packageType: 0,
        state
...
async function subTaskList ({appType, appAreas, packageType, state, vUserId, page, row, groupId}) {
    const res = await axios.post(`/subtask/list/${page}/${row}`, JSON.stringify({
        appType,
        appAreas,
        packageType,
        state,
...
async function CreateChildTask (taskInfo, state) {
    ...
    const res = await axios.post(`/subtask/save/${state}`, {
        ...taskInfo,
        modelStyle,
        chartlatFormat,
        chartlatProperty1,
...
async function CreateChildTaskDraft (taskInfo, state) {
    ...
    let entryEndTime = moment(new Date(rangeTime[0])).format("YYYY-MM-DD HH:mm:ss");
    let taskEndTime = moment(new Date(rangeTime[1])).format("YYYY-MM-DD HH:mm:ss");
    const res = await axios.post(`/subtask/savetemplet/${state}`, {
        ...taskInfo,
        modelStyle,
        chartlatFormat,
        chartlatProperty1,
...
async function subTaskDraftList () {
    const res = await axios.post('/subtask/queryTempletList');
    return res.data.data;
}
...
async function UpdateChildTask (taskInfo, state) {
    ...
    let entryEndTime = moment(new Date(rangeTime[0])).format("YYYY-MM-DD HH:mm:ss");
    let taskEndTime = moment(new Date(rangeTime[1])).format("YYYY-MM-DD HH:mm:ss");
    const res = await axios.post(`/subtask/update/${state}`, {
        ...taskInfo,
        modelStyle,
        chartlatFormat,
        chartlatProperty1,
...
//根据总任务查固定状态的报价单列表
async function taskOfferList (id) {
    const res = await axios.post(`/subtask/queryListByState`)
    return res.data;
}

//根据总任务固定状态的子任务导出excel
async function taskDownloadExcel ({task_id, state}) {
    const res = await axios.post(`/subtask/downloadExcel`, {task_id, state})
    return res.data;
}
...
async function entryList ({page, row, appType, appAreas, packageType, state = 4, vUserId}) {
    const res = await axios.post(`/subtask/entryList/${page}/${row}`, JSON.stringify({
        appType,
        appAreas,
        packageType,
        state,
        entryVUserId: vUserId
    }))
    return res.data.data;
}
